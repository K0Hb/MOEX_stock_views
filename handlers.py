...
@dp.message_handler(commands=['start', 'старт', 'вернуться'])
...
async def stocks_view(message):
    ...
    await message.answer(
        text=text, reply_markup=generate_keyboard('акции', True))
...
async def commodites_view(message):
    text = 'Выберите товар'
    await message.answer(
        text=text, reply_markup=generate_keyboard('товары', True))
...
async def indexes_view(message):
    ...
    await message.answer(
        text=text, reply_markup=generate_keyboard('индексы', True))
...
async def currencies_view(message):
    ...
    await message.answer(
        text=text, reply_markup=generate_keyboard('валютные_пары', True))


@dp.message_handler(commands=['нефть_brent', 'нефть_WTI',
                              'натуральный_газ', 'серебро',
                              'золото', 'палладий'])
...
@dp.message_handler(
    commands=['сбер', 'газпром', 'лукойл', 'яндекс', 'норильский_никель',
              'новатэк', 'татнефть', 'мосбиржа', 'втб', 'сургутнефтегаз',
              'алроса'])
...
@dp.message_handler(commands=['РТС', 'Мосбиржа_индекс', 'SP500', 'NASDAQ',
                              'DJI', 'DAX', 'IBOVESPA', 'KOSPI', 'Nikkei'])
async def view_index(message):
    text = get_index(indexs[message.text])
    await message.answer(text=text)


@dp.message_handler(commands=['USD/RUB', 'EUR/RUB', 'RUB/CNY', 'EUR/USD',
